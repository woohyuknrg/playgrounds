...
/*:
Instead, you can use specialized `String.Index` objects to index and slice strings. Every string has a `startIndex` and an `endIndex`, and you can call the `successor()` and `predecessor()` functions on the indexes to increment or decrement them, respectively. You can also use the `advance` function to offset an index–the integer you pass in is used to determine how many times to invoke `successor` or `predecessor`. The `String.Index` type is Unicode-aware, so the `advance` function will always advance a string index correctly. The `advance` function takes into account any large or complex Unicode characters to make sure your range starts, advances, and ends on a complete, user-perceived character.
...
Try option-clicking the `range` constant. The `advance` function returns a value with the same type as the index you pass it, so the `...` range operator produces a range of that type.
*/
let range = advance(str.startIndex, 4)...advance(str.startIndex, 12)
/*:
Because the `advance` function offsets any index by a specified integer amount, you can use the `advance` function to retrieve a character at a certain index in a string.
*/
str[advance(str.startIndex, 7)]
visualize(str, index: advance(str.startIndex, 7))
...
The app needs to indicate when a user's message is over the character limit. In the code below, the closure passed to the `showChatView` function uses a special version of the `advance` function that takes an end index in addition to the start index and offset as you've already seen previously. This version of the `advance` function increments the `startIndex` up to the `endIndex` without overrunning the end. If the string is less than 30 characters long (the value of `messageLimit`), the `advance` function won't increment past the end of the string. If it's over 30 characters, the function returns a range starting at the message limit and extending to the end of the message. The app takes the range returned by the function and highlights that range in red.
*/
let messageCharacterLimit = 30
showChatView { (contents: String) -> Range<String.Index>? in
    let range = advance(contents.startIndex, messageCharacterLimit, contents.endIndex)..<contents.endIndex
    return !range.isEmpty ? range : nil
}

/*:
  > **Experiment:**
  > Try changing the `messageCharacterLimit` variable. Notice how messages that are less than the character limit are handled properly by the `advance` function.
*/
/*:
  > **Checkpoint:**
